...
import { setUserWalletAddr } from 'redux/reducers/FormReducer';
...
export default function UserWalletAddress() {
    ...
    const { userWalletAddr } = useSelector((state) => state.FormReducer)
    ...
    const handleChange = (e) => {
        ...
        if (value.length <= 50) {
            dispatch(setUserWalletAddr(value))
        }
    }
    ...
    const handleMaxReset = () => {
        ...
        dispatch(setUserWalletAddr(false))
    }

    // Focus input element on submit if no value
    useEffect(() => {
        if (formError && formError.path === 'userWalletAddr') {
            ...
            <div className="w-full  px-4 mt-5">
                <div className="relative w-full mb-3">
                    ...
                    <div className="flex justify-between mb-2">
                        <p className="mb-2 text-sm">Your USDT wallet Address</p>
                    ...
                    />
                    {formError && formError.path === 'userWalletAddr' ?
