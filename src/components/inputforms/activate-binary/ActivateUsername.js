...
import { setActivationUsername, setIsUserHasChecked } from 'redux/reducers/ActivationReducer'
...
export default function ActivateUsernamed() {
    ...
    const [setNewUserData] = useState(false)
    const { isLogin, isBinary, username, token } = useSelector((state) => state.AuthReducer)
    const { activateUsername } = useSelector((state) => state.ActivationReducer)
    ...
    useEffect(() => {
        ...
        dispatch(setActivationUsername(false))
    ...
    return (
        <>
            <div className="w-full  mt-2">
                <p className="mb-2  text-white">Username to activate </p>
                    ...
                    <div className="w-5/12">
                        {spinnerCheck ?
                            ...
                            <button onClick={handleCheckDelay} className="_btn_select_epins hover:bg-blue-900 px-4 py-2 rounded-md ">
                                <i className="icofont-arrow-right"></i>  CHECK</button>
                        }
                    </div>
                </div>
